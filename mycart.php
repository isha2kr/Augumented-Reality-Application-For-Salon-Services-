...
if (isset($_REQUEST['uid'])) {
	...
	$user2 = mysqli_real_escape_string($dbhandle,$_REQUEST['uid']);
	if($user != $user2){
...
if (isset($_REQUEST['cid'])) {
		$cid = mysqli_real_escape_string($dbhandle,$_REQUEST['cid']);
		if(mysqli_query($dbhandle,"DELETE FROM orders WHERE pid='$cid' AND uid='$user'")){
...
//triming name
	try {
		...
		// Check if email already exists
						...
						$result = mysqli_query($dbhandle,"SELECT * FROM cart WHERE uid='$user'");
						...
						while ($get_p = mysqli_fetch_assoc($result)) {
							...
							mysqli_query($dbhandle,"INSERT INTO orders (uid,pid,quantity,oplace,mobile,odate) VALUES ('$user','$pid',$num,'$_POST[address]','$_POST[mobile]','$d')");
						}
							
						if(mysqli_query($dbhandle,"DELETE FROM cart WHERE uid='$user'")){
						...
						<div>
							<table class="rightsidemenu">
								...
								<tr>
									...
									$run = mysqli_query($dbhandle,$query);
									$total = 0;
									while ($row=mysqli_fetch_assoc($run)) {
										...
										$run1 = mysqli_query($dbhandle,$query1);
