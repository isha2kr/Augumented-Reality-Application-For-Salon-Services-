...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$getposts5 = mysqli_query($dbhandle,"SELECT * FROM orders WHERE id='$eoid'") or die(mysql_error());
	...
	$result1 = mysql_iquery($dbhandle,"SELECT * FROM orders WHERE id='$eoid'");
		$get_order_info = mysqli_fetch_assoc($result1);
			...
			$result2 = mysqli_query($dbhandle,"SELECT * FROM user WHERE id='$eouid'");
...
$getposts = mysqli_query($dbhandle,"SELECT * FROM products WHERE id ='$eopid'") or die(mysql_error());
	...
	try {
		if(empty($_POST['dstatus'])) {
			throw new Exception('Status can not be empty');
				...
				if(mysqli_query($dbhandle,"UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'")){
					//success message
...
//triming name
	if(mysqli_query($dbhandle,"DELETE FROM orders WHERE id='$eoid'")){
