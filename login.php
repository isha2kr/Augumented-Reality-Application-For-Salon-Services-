...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		$user_login = mysqli_real_escape_string($dbhandle,$_POST['email']);
		$user_login = mb_convert_case($user_login, MB_CASE_LOWER, "UTF-8");	
		$password_login = mysqli_real_escape_string($dbhandle,$_POST['password']);		
		$num = 0;
		$password_login_md5 = md5($password_login);
		$result = mysqli_query($dbhandle,"SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='yes'");
		...
		if ($num>0) {
			...
			if (isset($_REQUEST['ono'])) {
				$ono = mysqli_real_escape_string($dbhandle,$_REQUEST['ono']);
			...
			$result1 = mysqli_query($dbhandle,"SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='no'");
